...
### Q. アスタリスクになるんだけど
A. [TrJpMod](https://steamcommunity.com/sharedfiles/filedetails/?id=2865024243)など、フォントが同梱されているリソースパックやModを導入してください。
...
それでもエラーが解消しない場合、原因と思われるModのアップデートが正常に反映されていないことがあります。
一度「サブスクライブ解除 → 再度サブスクライブ」をお試しください。
改善しない場合は、一度該当Modファイルをすべて削除し、再導入してみてください。　　

**Modのワークショップファイルの場所：**  
Steam Workshopの原因と思われるModページを開き、URLの末尾の数字をコピーしてください   
例：Calamity Modの場合  
https://steamcommunity.com/sharedfiles/filedetails/?id=2824688072 の**2824688072**部分  
下記の場所に、コピーした数字と同じ名前のフォルダがあります  
Steam/steamapps/workshop/content/1281930/(コピーした数字)  

それでも問題が解消しない場合は、公式Discordにてエラーのスクリーンショットなどを添付した上でお問い合わせください。
なお、tModLoaderのエラーメッセージは内容が曖昧なことがあります。
たとえば「バージョンエラー」と表示されても、実際はHJSONの構文エラーが原因だった、というケースなど。
